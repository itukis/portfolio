...
// スワイプナビゲーションのセクション順（hero含む全セクション）
...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    const onTouchEnd = e => {
      ...
      touchStartX.current = null;
      isSwiping.current   = false;
      if (Math.abs(dx) < 50) return; // 閾値未満は無視
      goRelative(dx < 0 ? 1 : -1);  // 左スワイプ→次、右スワイプ→前
    };
  ...
  /* ── Current section index for swipe indicator ── */
  ...
  return (
    <div style={{ background: "#080810", minHeight: "100vh" }}>
      ...
      {/* ─── Scroll progress bar ─── */}
      <div className="fixed top-0 left-0 z-[60] h-[2px] pointer-events-none transition-all duration-150"
        style={{
          width: `${scrollPct}%`,
